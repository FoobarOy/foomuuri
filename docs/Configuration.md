...
`recursion_limit` is the internal limit to avoid template expansion loop.
...
Zone-zone section `localhost-localhost` (aka loopback
traffic, aka `127.0.0.1` and `::1`) is special case. It's final rule is
`accept`. Usually there is no need to add `localhost-localhost`section.
...
Refresh interval can be configured globally or
...
define list of rules, while macro defines single rule. Example:
...
These sections can be used to specify more rules to them.
